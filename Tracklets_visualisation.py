...
nb = 14
for s in range(2,nb):
    tracking_data_partial_filename = dst_folder+"/tracking_data_partially_observed_subset"+str(s+1)+".csv";
    tracking_data_partial = np.loadtxt(tracking_data_partial_filename, delimiter=",")
        ...
        for idx in range(1,nP_h+nP_a+1):
            if not np.isnan(tracking_data_partial[j,2*idx]):
                if idx == 1:
                    ...
                    team = 1;  #team 1 player
                elif idx == nP_a+1:
                    team = 4; #GK2
                else:
                    team = 3;  #team 2 player
